...
if [[ -d "$FFBUILD_DESTDIR" ]]; then
    ...
    # Strip plain .a archives only in non-debug builds. --strip-unneeded
    # removes debug data/local symbols too, so debug dependencies must stay intact.
    if [[ "$ADDINS_STR" != *debug* ]]; then
